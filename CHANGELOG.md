...
### Fixed
- Settings window launched by the system (e.g. Gatekeeper "Open Anyway" on first run) now renders correctly instead of appearing blank. The SwiftUI `Settings` scene hosted `EmptyView`; it now hosts the real `SettingsView`.
- Menu bar "Settings…" window now sizes correctly to its content.
